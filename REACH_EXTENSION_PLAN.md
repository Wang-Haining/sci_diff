...
## Signal-first pilot, authorized 2026-09-14

The user stopped the full fits to inspect a cheap signal before further investment. Jobs 240851/240875/240876 were cancelled; all prepared data and saved models remain intact. Run `reach_extension_pilot.py` using the full saved populations and propensity, without new model fitting. Its Hajek IPW means, breadth, distance shares, and paired follow-up contrasts are exploratory and not interchangeable with AIPW. Journal intervals condition on the saved weights. New files belong only in `results/reach_extension_pilot_v1`; manuscript results stay unchanged. Compare the five-year IPW routing contrast with the saved downstream AIPW scores to make estimator sensitivity visible. Keep null and contrary signals; select worthwhile follow-up questions by substantive information, not significance. Do not resume full refitting without user review.
